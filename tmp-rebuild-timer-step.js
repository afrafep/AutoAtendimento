...
const addAfter = (search, addition) => {
  if (s.includes(addition.trim())) return;
  const idx = s.indexOf(search);
  if (idx === -1) throw new Error("Nao encontrou ancora");
  s = s.slice(0, idx + search.length) + addition + s.slice(idx + search.length);
};

const replaceText = (search, replacement) => {
  if (!s.includes(search)) throw new Error("Nao encontrou replace");
