...
if (!s.includes("const [consultaTecladoTokenId")) {
  s = s.replace(
...
if (!s.includes("const abrirTecladoTokenInline")) {
  s = s.replace(
...
if (!s.includes("const preencherTokenViaTecladoInline")) {
  s = s.replace(
...
if (!s.includes("const tecladoTokenAberto = consultaTecladoTokenId === consulta.idEvento;")) {
  s = s.replace(
...
if (!s.includes("onFocus={() => abrirTecladoTokenInline(consulta.idEvento, indiceToken)}")) {
  s = s.replace(
    /(onPaste=\{\(event\) =>\s*\n\s*handleTokenInlinePaste\(consulta\.idEvento, event\)\s*\n\s*\})/,
    `$1
...
if (!s.includes("TECLADO NUMÉRICO")) {
  s = s.replace(
    /\n\s*\{tokenErro \? \(/,
    `
...
s = s.replace(
  /setConsultaTokenAbertaId\(null\);\n\s*await buscarConsultas\(\);/,
  "setConsultaTokenAbertaId(null);\n    setConsultaTecladoTokenId(null);\n    await buscarConsultas();",
);

s = s.replace(/Atendimento indisponÃƒÆ’Ã‚Â­vel/g, "Atendimento indisponível");
