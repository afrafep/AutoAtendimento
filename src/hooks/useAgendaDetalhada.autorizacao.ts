...
import {
  enviarParaPacsAposAutorizacao,
  formatarDataAutorizacao,
  formatarHoraAutorizacao,
  getEmojiPacienteAutorizacao,
  prepararContextoAutorizacaoAgrupada,
} from "./useAgendaDetalhada.autorizacao.helpers";
import {
...
interface MarcacaoAutorizacaoContext {
  agenda: AgendaEvento[];
  fetchAgenda: () => Promise<void>;
  atualizarStatusEmLote: (idEventoAlterado: number, novoStatus: string) => Promise<void>;
  persistirDadosGuiaAutorizacao: (
    idEvento: number,
    numeroGuiaGerado: string,
    numeroGuiaOperadora: number,
  ) => void;
  obterDadosGuiaAutorizacao: (evento: AgendaEvento) => DadosGuiaAutorizacao;
  encaminharSenhaPainelParaMedico: (evento: AgendaEvento) => Promise<void>;
}

export const createAgendaDetalhadaAutorizacao = ({
  agenda,
  fetchAgenda,
  ...
  const marcarAutorizacao = async (
    ...
    const formatarData = formatarDataAutorizacao;
    const formatarHora = formatarHoraAutorizacao;
    const getEmojiPaciente = getEmojiPacienteAutorizacao;
    ...
    } = prepararContextoAutorizacaoAgrupada(agenda, evento);
  ...
  `,
    ...
    `,
      ...
      try {
        ...
        await enviarParaPacsAposAutorizacao({
          evento,
          eventoBaseAutorizacao,
          itensParaPacsRaw,
          isUltrassom,
